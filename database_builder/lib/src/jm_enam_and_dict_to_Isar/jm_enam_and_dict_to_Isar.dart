...
Future<bool> jmEnamAndDictToIsar(Isar isar) async {
    ...
    if (isar.jmdict.countSync() <= 0)
    {
      ...
      List jmdict = jsonDecode(await File(p.join(RepoPathManager.getPartiallyProcessedFilesPath(), 'JMdict', 'jmdict.json')).readAsString());
    ...
    if(isar.jmnedict.countSync() <= 0){
      ...
      List jmnedict = jsonDecode(await File(p.join(RepoPathManager.getPartiallyProcessedFilesPath(), 'JMdict', 'jmnedict.json')).readAsString());
      print("Json read, putting objects to box");
